...
def process_pdf(pdf_path: Path, output_dir: Path = None):
    ...
    if output_dir is None:
        output_dir = Path(".")  # Use current directory as default if output_dir is None
    else:
...
if __name__ == "__main__":
    pdf_file_path = Path("ocr2025-03-31T15-27-50.pdf")
    output_directory = Path("output")  # Create a directory named "output"
    output_directory.mkdir(exist_ok=True) # Ensure the directory exists
    process_pdf(pdf_file_path, output_dir=output_directory)